...
export const areAllValsFalse = (vals: TFlatSubscriptionStore): boolean => {
  for (const key in vals) {
    if (vals[key as keyof TFlatSubscriptionStore]) {
      // console.log(`value is not false`, key, vals[key as keyof TFlatSubscriptionStore]);
      return false;
    }
  }
  return true;
